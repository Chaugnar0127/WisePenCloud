...
public enum DocumentError implements IResult {
    ...
    DOCUMENT_EDIT_SESSION_ACTIVE(6151, new ResultKey(BusinessDomain.DOCUMENT, DocumentSubject.DOCUMENT, ErrorReason.STATE_INVALID),"文档正在协同编辑，不能上传新版本"),
    DOCUMENT_EDIT_NOT_SUPPORTED(6152, new ResultKey(BusinessDomain.DOCUMENT, DocumentSubject.DOCUMENT, ErrorReason.UNSUPPORTED),"该文档类型暂不支持在线编辑"),
    DOCUMENT_EDIT_SESSION_NOT_FOUND(6153, new ResultKey(BusinessDomain.DOCUMENT, DocumentSubject.DOCUMENT, ErrorReason.NOT_FOUND),"文档编辑会话不存在或已过期"),
    DOCUMENT_EDIT_CALLBACK_INVALID(6154, new ResultKey(BusinessDomain.DOCUMENT, DocumentSubject.DOCUMENT, ErrorReason.PERMISSION_DENIED),"文档编辑回调校验失败"),
