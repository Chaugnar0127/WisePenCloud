...
import com.oriole.wisepen.resource.domain.dto.ResourceOriginalEditorsUpdateReqDTO;
...
public class NoteVersionServiceImpl implements INoteVersionService {
    ...
    private List<Long> updateNoteDocumentMetadata(NoteSnapshotMessage msg, List<Long> currentRoundAuthors) {
        return noteDocumentRepository.findByResourceId(msg.getResourceId()).map(noteInfo -> {
            List<Long> newAuthors = Collections.<Long>emptyList();
            // 更新最后修改时间
    ...
    private void updateResourceOriginalEditors(String resourceId, List<Long> newAuthors) {
        ...
        remoteResourceService.addOriginalEditors(ResourceOriginalEditorsUpdateReqDTO.builder()
                .resourceId(resourceId)
                .originalEditorIds(newAuthors.stream().map(String::valueOf).toList())
                .build());
    }
